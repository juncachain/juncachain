...
func TestRewardBalance(t *testing.T) {
	contractBackend := backends.NewSimulatedBackend(core.GenesisAlloc{
		acc1Addr: {Balance: new(big.Int).SetUint64(100000000000000000)},
		acc2Addr: {Balance: new(big.Int).SetUint64(100000000000000000)},
		acc4Addr: {Balance: new(big.Int).SetUint64(100000000000000000)},
	}, 10000000)

	head, _ := contractBackend.HeaderByNumber(context.Background(), nil) // Should be child's, good enough
	gasPrice := new(big.Int).Add(head.BaseFee, big.NewInt(1))

	acc1Opts, _ := bind.NewKeyedTransactorWithChainID(acc1Key, big.NewInt(1337))
	acc2Opts, _ := bind.NewKeyedTransactorWithChainID(acc2Key, big.NewInt(1337))
	...
	validatorCap := new(big.Int)
	validatorCap.SetString("50000000000000000000000", 10)
	validatorAddr, _, baseValidator, err := contractValidator.DeployJuncaValidator(
		transactOpts,
		contractBackend,
		[]common.Address{addr},
	...
	foundationAddr := common.HexToAddress(params.JuncaFoudation)
...
func GetCandidatesOwnerBySigner(validator *contractValidator.JuncaValidator, signerAddr common.Address) common.Address {
	owner := signerAddr
	opts := new(bind.CallOpts)
