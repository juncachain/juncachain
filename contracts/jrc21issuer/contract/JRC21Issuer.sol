...
contract JRC21Issuer{
    ...
    mapping(address => uint256) private _gasLefts;
    mapping(address => address) private _issuers;
    ...
    function gasLeft(address token)public view returns(uint256){
        return _gasLefts[token];
    }

    function issuer(address token)public view returns(address){
        return _issuers[token];
    }
