...
interface IJuncaswapV2Callee {
    function juncaswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external;
}
...
contract JuncaswapV2ERC20 is IJuncaswapV2ERC20 {
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'JuncaswapV2: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'JuncaswapV2: INVALID_SIGNATURE');
        _approve(owner, spender, value);
    }
}

contract JuncaswapV2Pair is IJuncaswapV2Pair, JuncaswapV2ERC20 {
    using SafeMath  for uint;
    ...
    modifier lock() {
        require(unlocked == 1, 'JuncaswapV2: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    uint private mtv1; 				        // mint token1 transact value when sold
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'JuncaswapV2: TRANSFER_FAILED');
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'JuncaswapV2: OVERFLOW');
            ...
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
					...
					if(gasless){
						...
						uint liquidity = numerator / denominator;
						if (liquidity > 0) _mint(issuer, liquidity);
					}
                }
            }
        } else if (_kLast != 0) {
    ...
    function mint(address to) external lock authCaller returns (uint liquidity) {
        ...
        require(liquidity > 0, 'JuncaswapV2: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock authCaller returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'JuncaswapV2: INSUFFICIENT_LIQUIDITY_BURNED');
...
contract JuncaswapV2Factory is IJuncaswapV2Factory {
    ...
    event PairCreated(address indexed token0, address indexed token1, address pair, uint ,bool);
    ...
    function createPair(address tokenA, address tokenB,bool _gasless) external returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), 'JuncaswapV2: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(JuncaswapV2Pair).creationCode;
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'JuncaswapV2: FORBIDDEN');
        feeTo = _feeTo;
    }

	function setGasPerTx(uint _gasPerTx) external {
        require(msg.sender == feeToSetter, 'JuncaswapV2: FORBIDDEN');
        gasPerTx = _gasPerTx;
    }
